...
def Routine():
    ...
    outfile = outfile + " (curvas).swb"     #nomeia os arquivos de saída
    try:
        os.remove(outfile)
            ...
            for foil in airfoil_files:                      #Para cada arquivo recebido, executar este código
                ...
                with open(foil, 'r') as i:
                    ...
                    while True:                                 #Aqui começa a leitura e manejamento do arquivo
                        ...
                        x = content.split(",")                  #Divida a string em uma lista, com o delimitador sendo ","
                        x[0] = float(x[0])/1000                 #Transforme a string em um float, a partir daqui sendo considerado como um número normal, e divida por 1000
                        x[1] = float(x[1])/1000
                        x[2] = float(x[2])/1000
