...
class PFoil:
    ...
    def write_file2(self, file = 'airfoils\\airfoil.txt', npTotal = 100, npTE = 15, npLE = 15, npID = 10, npED = 60, anchorL = 0.1, anchorT = 0.9):
        done = write_from_coeffs_new(self.aup, self.alo, file, npTotal = npTotal, npTE = npTE, npLE = npLE, npID = npID, npED = npED, anchorL = anchorL, anchorT = anchorT)
        return done
...
def write_from_coeffs_new(aup, alo, coord_file, npTotal = 100, npTE = 15, npLE = 15, npID = 10, npED = 60, anchorL = 0.1, anchorT = 0.9):
    npTE = round(npTE*npTotal/200)
    npLE = round(npLE*npTotal/200)
    npID = round(npID*npTotal/100)
    npED = round(npED*npTotal/100)
    try:
        with open(coord_file, 'w') as inp:
