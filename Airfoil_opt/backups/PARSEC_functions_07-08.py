...
class PFoil:
    def __init__(self, random = False, selig_file = "", plot_diff = False, params_vec = []):
        if selig_file != "":
    ...
    def write_file(self, file = 'airfoils\\airfoil.txt'):
        done = write_from_coeffs(self.aup, self.alo, file)
        return done

    def write_file2(self, file = 'airfoils\\airfoil.txt'):
        done = write_from_coeffs_new(self.aup, self.alo, file)
        return done
...
def write_from_coeffs(aup, alo, coord_file, np = 100):
    try:
        with open(coord_file, 'w') as inp:
            for i in range(1, np + 1):
                theta = (180.0/(np - 1))*(np - i)
                x = 0.5 - (0.5*nup.cos(math.radians(theta)))
...
def write_from_coeffs_new(aup, alo, coord_file, np = 150, anchor = 0.05):
    try:
        with open(coord_file, 'w') as inp:
            for xx in nup.linspace(1, 0.1, 15, endpoint = False):
                inp.write(f'{xx} {main_func(alo, xx)}\n')
            for i in range(np - 2, 0, -1):
                theta = (180.0/(np - 1))*(np - i)
                #theta = (90/(np - 1))*(np - i)
                xx = anchor + (anchor*nup.cos(math.radians(theta)))
                inp.write(f'{xx} {main_func(alo, xx)}\n')
            ...
            for i in range(1, np - 1):
                ...
                #theta = (90/(np - 1))*(np - i)
                xx = anchor + (anchor*nup.cos(math.radians(theta)))
                inp.write(f'{xx} {main_func(aup, xx)}\n')
            for xx in nup.linspace(0.1, 0.8, 50, endpoint = False):
                inp.write(f'{xx} {main_func(aup, xx)}\n')
            for xx in nup.linspace(0.8, 1, 5, endpoint = True):
                inp.write(f'{xx} {main_func(aup, xx)}\n')
        return True
    except:
        return False

def write_from_coeffs_new_new(aup, alo, coord_file, npTE = 15, npLE = 15, npID = 10, npED = 60, anchorL = 0.1, anchorT = 0.9):
    try:
        with open(coord_file, 'w') as inp:
            for i in range(0, npTE):
                theta = (90/npTE)*(npTE - i)
                xx = anchorT + ((1 - anchorT)*nup.sin(math.radians(theta)))
                inp.write(f'{xx} {main_func(alo, xx)}\n')
            for xx in nup.linspace(anchorT, anchorL, npID, endpoint = False):
                inp.write(f'{xx} {main_func(alo, xx)}\n')
            for i in range(npLE, 0, -1):
                theta = (90/npLE)*(npLE - i)
                xx = anchorL - (anchorL*nup.sin(math.radians(theta)))
                inp.write(f'{xx} {main_func(alo, xx)}\n')
            for i in range(0, npLE):
                theta = (90/npLE)*(npLE - i)
                xx = anchorL - (anchorL*nup.sin(math.radians(theta)))
                inp.write(f'{xx} {main_func(aup, xx)}\n')
            for xx in nup.linspace(anchorL, anchorT, npED, endpoint = False):
                inp.write(f'{xx} {main_func(aup, xx)}\n')
            for i in range(npTE, 0, -1):
                theta = (90/npTE)*(npTE - i)
                xx = anchorT + ((1 - anchorT)*nup.sin(math.radians(theta)))
                inp.write(f'{xx} {main_func(aup, xx)}\n')
                return True
...
def check_too_thin(aup, alo, thick_min = 0.02):
    for x in nup.linspace(0.1, 0.9, 50):
        zu = main_func(aup, x)
        zlo = main_func(alo, x)
        if zu - zlo < thick_min:
            return True
...
def coeffs_from_curves(x_lo, x_u, y_lo, y_u):
    x_mat_lo = nup.ones((x_lo.size, 6))
    for i in range(len(x_lo)):
        for ii in range(6):
            x_mat_lo[i][ii] = x_lo[i]**(ii + 0.5)
    x_mat_u = nup.ones((x_u.size, 6))
