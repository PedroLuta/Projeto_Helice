...
class NSGA2:
    ...
    def create_offspring(self):
        ...
        while len(chrom_pop) < length:
            mother = select_tournament(population, tournament_size = self.t_size)
            father = select_tournament(population, tournament_size = self.t_size)
            son_chrom = crossoverFull(mother.get_chrom(), father.get_chrom())
            son_chrom = mutate(son_chrom, self.mut_rate, self.limits, decimal_places = self.decimal_places)
    ...
    def generate_chrom(self):
        ...
        for key in self.limits:
            param = Auxiliary.generate_random_parameter(self.limits[key], self.decimal_places)
            chrom[key] = param
        return chrom

    def assign_fronts(self):
        copy = self.current_pop[:]
        n_objvals = len(copy[0].get_ObjVal())
        new_pop = []
        front = 1
        while len(copy) > 0:
...
def mutate(chrom, mut_rate, limits, decimal_places = 4):
    ...
    for key in copy:
        mut = random.random()
        if mut < mut_rate:
            param = Auxiliary.generate_random_parameter(limits[key], decimal_places)
            copy[key] = param
    return copy
